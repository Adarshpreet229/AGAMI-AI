...
async def generate_images(prompt: str):
    ...
    for _ in range(4):
        ...
        tasks.append(task)  # Fixed typo: Changed `task.append(task)` to `tasks.append(task)`
...
while True:
    try:
        ...
        # Split the data by comma and strip any leading/trailing spaces
        prompt, status = [part.strip() for part in data.split(",")]

        # If the status indicates an image generation request
        if status == "True":
            print("Generating Images...")
            GenerateImages(prompt=prompt)  # Fixed typo: Changed `ImageStatus` to `GenerateImages`
