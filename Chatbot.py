...
import datetime  # Importing the datetime module for real-time and time.
from dotenv import dotenv_values  # Importing dotenv_values to read environment variables from a .env file.

# Load environment variables from the .env file.
env_vars = dotenv_values(".env")  # Fixed the filename (removed space).
...
# Check if the API key is retrieved properly.
if not GroqAPIKey:
...
*** Reply in only English, even if the question is in Hindi or any other language, reply in English.***
...
try:
    ...
    # If the file doesn't exist, creat an empty JSON file to store chat logs.
    with open(r"Data\ChatLog.json", "w") as f:
        dump([], f)
        
# Function to get real_time date and time information.
